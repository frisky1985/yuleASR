#!/bin/bash
#============================================================================
# build.sh - C10: QEMU WDG Timeout Reset Verification
# Usage: ./build.sh [run]
#============================================================================
set -euo pipefail
...
APP_SRCS="$QM33/src/startup_m33.s $QM33/src/Uart_Cfg.c $QM33/src/hooks.c $QM33/src/libc_stubs.c $COMMON/qemu_assert.c $SCRIPT_DIR/wdg_qemu_stub.c $SCRIPT_DIR/main_wdg_timeout.c"

OUT="$SCRIPT_DIR/qemu_wdg_timeout.elf"
echo "==> Building $OUT ..."
...
if [ "${1:-}" = "run" ]; then
    ...
    "$SCRIPT_DIR/../ci/run_qemu_test.sh" "$OUT" "QEMU_FULL_STACK_PASS" "/tmp/c10_wdg_timeout.log"
fi
echo "==> Build OK"
