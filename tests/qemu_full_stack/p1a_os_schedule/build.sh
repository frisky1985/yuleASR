#!/bin/bash
#============================================================================
# build.sh - C2: QEMU OS Schedule Verification
# Usage: ./build.sh [run]
#============================================================================
set -euo pipefail
...
APP_SRCS="$QM33/src/startup_m33.s $QM33/src/Uart_Cfg.c $QM33/src/hooks.c $QM33/src/libc_stubs.c $COMMON/qemu_assert.c $SCRIPT_DIR/main_os_schedule.c"

OUT="$SCRIPT_DIR/qemu_os_schedule.elf"
echo "==> Building $OUT ..."
$CROSS $CFLAGS $INCLUDES -T "$QM33/qemu_m33.ld" $KERNEL_SRCS $APP_SRCS -L"$LIBGCC_NOFP" -lgcc -o "$OUT"
arm-none-eabi-size "$OUT"

if [ "${1:-}" = "run" ]; then
    "$SCRIPT_DIR/../ci/run_qemu_test.sh" "$OUT" "QEMU_FULL_STACK_PASS" "/tmp/c2_os_schedule.log"
fi
echo "==> Build OK"
