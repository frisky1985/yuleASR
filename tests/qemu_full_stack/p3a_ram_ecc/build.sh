#!/bin/bash
#============================================================================
# build.sh - C9: QEMU RAM ECC Fault Injection Verification
# Usage: ./build.sh [run]
#============================================================================
set -euo pipefail
...
APP_SRCS="$QM33/src/startup_m33.s $QM33/src/Uart_Cfg.c $QM33/src/hooks.c $QM33/src/libc_stubs.c $COMMON/qemu_assert.c $SCRIPT_DIR/ram_ecc_fault_inject.c $SCRIPT_DIR/main_ram_ecc.c"

OUT="$SCRIPT_DIR/qemu_ram_ecc.elf"
echo "==> Building $OUT ..."
...
if [ "${1:-}" = "run" ]; then
    ...
    "$SCRIPT_DIR/../ci/run_qemu_test.sh" "$OUT" "QEMU_FULL_STACK_PASS" "/tmp/c9_ram_ecc.log"
fi
echo "==> Build OK"
