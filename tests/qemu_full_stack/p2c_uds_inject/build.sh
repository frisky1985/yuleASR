#!/bin/bash
#============================================================================
# build.sh - C6: UDS Diagnostic Injection Verification
# Usage: ./build.sh [run]
#============================================================================
set -euo pipefail
...
APP_SRCS="$QM33/src/startup_m33.s $QM33/src/Uart_Cfg.c $QM33/src/hooks.c $QM33/src/libc_stubs.c $COMMON/qemu_assert.c $SCRIPT_DIR/uds_response_capture.c $SCRIPT_DIR/main_uds_inject.c"

OUT="$SCRIPT_DIR/qemu_uds_inject.elf"
echo "==> Building $OUT ..."
$CROSS $CFLAGS $INCLUDES -T "$QM33/qemu_m33.ld" $KERNEL_SRCS $APP_SRCS -L"$LIBGCC_NOFP" -lgcc -o "$OUT"
arm-none-eabi-size "$OUT"

if [ "${1:-}" = "run" ]; then
    "$SCRIPT_DIR/../ci/run_qemu_test.sh" "$OUT" "QEMU_FULL_STACK_PASS" "/tmp/c6_uds.log"
fi
echo "==> Build OK"
