#!/bin/bash
#============================================================================
# build.sh - C3: EcuM Startup Sequence Verification
# Usage: ./build.sh [run]
#============================================================================
set -euo pipefail
...
APP_SRCS="$QM33/src/startup_m33.s $QM33/src/Uart_Cfg.c $QM33/src/hooks.c $QM33/src/libc_stubs.c $COMMON/qemu_assert.c $SCRIPT_DIR/ecum_test_stubs.c $SCRIPT_DIR/main_ecum_startup.c"

OUT="$SCRIPT_DIR/qemu_ecum_startup.elf"
echo "==> Building $OUT ..."
$CROSS $CFLAGS $INCLUDES -T "$QM33/qemu_m33.ld" $KERNEL_SRCS $APP_SRCS -L"$LIBGCC_NOFP" -lgcc -o "$OUT"
arm-none-eabi-size "$OUT"

if [ "${1:-}" = "run" ]; then
    "$SCRIPT_DIR/../ci/run_qemu_test.sh" "$OUT" "QEMU_FULL_STACK_PASS" "/tmp/c3_ecum.log"
fi
echo "==> Build OK"
