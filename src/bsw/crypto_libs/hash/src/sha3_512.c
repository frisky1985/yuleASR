/***********************************************************************************************************************
 * File:        sha3_512.c
 * Description: SHA-3-512 implementation (FIPS 202, 512-bit digest)
 *              Keccak-f[1600] with 1024-bit capacity and 576-bit rate
 **********************************************************************************************************************/
...
#define SHA3_512_BLOCK_SIZE         72U     /* 576 bits = 9 * 64-bit lanes */
#define SHA3_512_DIGEST_SIZE        64U     /* 512 bits */
#define SHA3_512_CAPACITY           128U    /* 1024 bits */
...
void SHA3_512_Update(SHA3_512_Context* ctx, const uint8_t* data, uint32_t len)
{
    const uint8_t* d = data;
    uint32_t blocks;

    if (ctx->bufLen > 0) {
        uint32_t need = SHA3_512_BLOCK_SIZE - ctx->bufLen;
        if (len < need) {
        ...
        sha3_absorb(ctx->state, ctx->buffer, 1, SHA3_512_BLOCK_SIZE);
        d += need;
        len -= need;
        ctx->bufLen = 0;
    }

    blocks = len / SHA3_512_BLOCK_SIZE;
    if (blocks > 0) {
        sha3_absorb(ctx->state, d, blocks, SHA3_512_BLOCK_SIZE);
        d += blocks * SHA3_512_BLOCK_SIZE;
        len -= blocks * SHA3_512_BLOCK_SIZE;
    }
...
void SHA3_512_Final(SHA3_512_Context* ctx, uint8_t digest[64])
{
    uint32_t padLen = SHA3_512_BLOCK_SIZE - ctx->bufLen;
    ctx->buffer[ctx->bufLen] = 0x06;
    memset(ctx->buffer + ctx->bufLen + 1, 0, padLen - 2);
    ctx->buffer[SHA3_512_BLOCK_SIZE - 1] = 0x80;

    sha3_absorb(ctx->state, ctx->buffer, 1, SHA3_512_BLOCK_SIZE);
    sha3_squeeze(ctx->state, digest, SHA3_512_DIGEST_SIZE, SHA3_512_BLOCK_SIZE);

    memset(ctx, 0, sizeof(*ctx));
}

void SHA3_512(const uint8_t* data, uint32_t len, uint8_t digest[64])
{
