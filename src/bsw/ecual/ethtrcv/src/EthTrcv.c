...
typedef struct
{
    ...
    uint16 PhyRegCache[32];  /* Cache for PHY registers 0-31 */
...
{
    /* 0x0180 */ ETHTRCV_TYPE_TJA1100,    /* NXP TJA1100 */
    /* 0x001C */ ETHTRCV_TYPE_LAN8720,     /* SMSC/Microchip LAN8720 */
    /* 0x0007 */ ETHTRCV_TYPE_DP83848,     /* TI DP83848 */
    /* 0x001C */ ETHTRCV_TYPE_KSZ8081,     /* Microchip KSZ8081 */
};
...
static Std_ReturnType EthTrcv_SetModeRTL8211(uint8 TrcvIdx, EthTrcv_ModeType Mode)
{
    ...
    switch (Mode)
    {
        ...
        default:
            /* RTL8211 doesn't support STANDBY/SLEEP modes */
            return E_NOT_OK;
    }
            ...
            switch (TrcvCfg->DetectedType)
            {
                ...
                case ETHTRCV_TYPE_RTL8211E:
                    ...
                    if (Status == E_OK)
                    {
                        ...
                        /* Parse duplex mode */
                        TrcvRuntime->DuplexMode = (Physr & ETHTRCV_RTL8211_PHYSR_DUPLEX) ? 
                ...
                case ETHTRCV_TYPE_LAN8742:
                    ...
                    if (Status == E_OK)
                    {
                        TrcvRuntime->BaudRate = (Physr & ETHTRCV_LAN8720_SCSR_SPEED) ? 
                                                 ETHTRCV_BAUD_RATE_100MBIT : ETHTRCV_BAUD_RATE_10MBIT;
                        TrcvRuntime->DuplexMode = (Physr & ETHTRCV_LAN8720_SCSR_DUPLEX) ? 
                                                   ETHTRCV_DUPLEX_MODE_FULL : ETHTRCV_DUPLEX_MODE_HALF;
                    }
                    break;
                ...
                default:
                    /* Generic PHY - try to parse from BMSR */
...
static void EthTrcv_ProcessLinkStateMachine(uint8 TrcvIdx)
{
    ...
    if (TrcvRuntime->LinkChangePending)
    {
        ...
        if (TrcvRuntime->LinkChangeCounter >= 2U)  /* Delay callback by 2 main function cycles */
        {
            ...
            {
                EthTrcv_LinkStateChgCallback(
                    EthTrcv_TrcvConfig[TrcvIdx].CtrlIdx,
                    TrcvRuntime->LinkState
                );
            }
#endif
        }
...
void EthTrcv_Init(const EthTrcv_ConfigType* CfgPtr)
{
    ...
    /* Configuration pointer can be NULL if using post-build configuration */
    (void)CfgPtr;
#endif
    ...
    for (TrcvIdx = 0U; TrcvIdx < ETHTRCV_NUMBER_OF_TRCVS; TrcvIdx++)
    {
            ...
            (EthTrcv_TrcvConfig[TrcvIdx].WakeupSource == WakeupSource))
        {
            ...
            switch (EthTrcv_TrcvConfig[TrcvIdx].DetectedType)
            {
                ...
                default:
                    /* Check generic link status change as wake-up */
