/**
 * @file LinNm.c
 * @brief LIN Network Management module implementation following AutoSAR Classic Platform 4.x
 * @version 1.0.0
 * @date 2026-05-05
...
static const LinNm_ConfigType* LinNm_ConfigPtr = NULL;
...
static void LinNm_SendNmPdu(uint8 channelIndex, boolean activeWakeup)
{
    ...
    if (chConfig->NodeType == LINNM_NODE_TYPE_MASTER) {
        ...
        (void)LinIf_ScheduleRequest(chConfig->LinIfChannelHandle, 1U); /* Diagnostic schedule */
    }
...
static void LinNm_ReceiveNmPdu(uint8 channelIndex, const uint8* pduData)
{
    ...
    if (!LinNm_IsChannelValid(channelIndex) || (pduData == NULL)) {
        return;
    }
    ...
    if ((chRuntime->State == LINNM_STATE_PREPARE_BUS_SLEEP) ||
        (chRuntime->State == LINNM_STATE_BUS_SLEEP)) {
...
static void LinNm_UpdateRemoteSleepIndication(uint8 channelIndex)
{
    ...
    if (chConfig->RemoteSleepIndEnabled) {
        ...
        if ((chRuntime->RemoteSleepTimer >= chConfig->RemoteSleepIndTimeMs) &&
            (!chRuntime->RemoteSleepIndication)) {
...
static void LinNm_NotifyModeChange(uint8 channelIndex, Nm_ModeType newMode)
{
    ...
    switch (newMode) {
        ...
        case NM_MODE_PREPARE_BUS_SLEEP:
            ...
            break;
        case NM_MODE_SYNCHRONIZE:
            /* LIN does not have explicit synchronize mode, map to network mode */
    ...
    switch (chRuntime->State) {
        ...
        case LINNM_STATE_READY_SLEEP:
            switch (event) {
                ...
                case LINNM_EVENT_MSG_RECEIVED:
                    /* Reset timeout timer */
                    LinNm_StartTimeoutTimer(channelIndex, 
                        LinNm_ConfigPtr->ChannelConfig[channelIndex].TimeoutTimeMs);
                    break;
                case LINNM_EVENT_TIMEOUT:
                    /* Enter prepare bus sleep */
        ...
        case LINNM_STATE_REPEAT_MESSAGE:
            switch (event) {
                ...
                case LINNM_EVENT_TIMEOUT:
                    ...
                    break;
                default:
                    /* Stay in repeat message, decrement counter in MainFunction */
                    break;
            }
            break;
...
void LinNm_Init(const LinNm_ConfigType* config)
{
    ...
    for (i = 0; i < LINNM_NUMBER_OF_CHANNELS; i++) {
        ...
        LinNm_ConfigPtr->ChannelRuntime[i].UserDataLength = 
            LinNm_ConfigPtr->ChannelConfig[i].UserDataLength;
...
Std_ReturnType LinNm_NetworkRequest(NetworkHandleType nmChannelHandle)
{
...
#endif
    {
        ...
        if (LinNm_ConfigPtr->ChannelRuntime[channelIndex].State == LINNM_STATE_NORMAL_OPERATION) {
            ret = E_OK; /* Already in network mode */
        } else {
            LinNm_StateMachine(channelIndex, LINNM_EVENT_NETWORK_REQUEST);
            ret = E_OK;
        }
...
#else
    (void)channelIndex; /* Suppress unused parameter warning */
#endif
...
void LinNm_MainFunction(void)
{
    ...
    for (i = 0; i < LINNM_NUMBER_OF_CHANNELS; i++) {
        ...
        if ((chRuntime->State == LINNM_STATE_NORMAL_OPERATION) ||
            (chRuntime->State == LINNM_STATE_REPEAT_MESSAGE)) {
...
void LinIf_TxConfirmation(uint8 Channel, uint8 LinTxPduId)
{
    ...
    (void)LinTxPduId; /* May be used for future extensions */
    ...
    for (i = 0; i < LINNM_NUMBER_OF_CHANNELS; i++) {
        if (LinNm_ConfigPtr->ChannelConfig[i].LinIfChannelHandle == Channel) {
            ...
            /* Reset message cycle timer */
            LinNm_ConfigPtr->ChannelRuntime[i].MessageCycleTimer = 
                LinNm_ConfigPtr->ChannelConfig[i].MsgCycleTimeMs;
            break;
        }
    }
}
...
void LinIf_RxIndication(uint8 Channel, uint8 LinRxPduId, uint8* SduPtr)
{
    ...
    (void)LinRxPduId; /* May be used for future extensions */
...
void LinIf_ScheduleRequestConfirmation(uint8 Channel, uint8 ScheduleIndex)
{
    ...
    (void)ScheduleIndex; /* May be used for future extensions */
