...
/**
 * CRC32 lookup table (ISO 3309 / AUTOSAR standard polynomial 0x04C11DB7).
 * Small enough to keep in ROM; no hardware dependency.
 */
/** CRC32 table (polynomial 0x04C11DB7, reflected 0xEDB88320) */
...
static uint32_t crc32_update(uint32_t crc, const uint8_t *data, uint32_t len)
{
    for (uint32_t i = 0U; i < len; i++) {
        crc = (crc << 8) ^ crc32_table[((crc >> 24) ^ data[i]) & 0xFFU];
    }
    return crc;
}
                                   ...
                                   Boot_ImageTrailer *out_trailer)
{
    ...
    return Boot_Flash_Read(trailer_addr,
                           (uint8_t *)out_trailer,
                           sizeof(Boot_ImageTrailer));
}
