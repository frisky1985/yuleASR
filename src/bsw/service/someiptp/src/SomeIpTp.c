...
Std_ReturnType SomeIpTp_CancelTransmit(PduIdType TxPduId)
{
    ...
    if (SomeIpTp_InternalState.State != SOMEIPTP_STATE_INIT)
    {
        SOMEIPTP_DET_REPORT_ERROR(SOMEIPTP_SID_CANCELTRANSMIT, SOMEIPTP_E_UNINIT);
        return E_NOT_OK;
    }
#endif
...
void SomeIpTp_RxIndication(PduIdType RxPduId, const PduInfoType* PduInfoPtr)
{
    ...
    if (SomeIpTp_FindChannelByRxPduId(RxPduId, &channelId) == E_OK)
    {
        SomeIpTp_ProcessReassembly(channelId, PduInfoPtr);
    }
}
...
void SomeIpTp_TxConfirmation(PduIdType TxPduId, Std_ReturnType result)
{
    ...
    if (SomeIpTp_FindChannelByTxPduId(TxPduId, &channelId) == E_OK)
    {
        ...
        if (channelPtr->State == SOMEIPTP_CHANNEL_TX_WAIT_CONFIRM)
        {
            ...
            /* Send next segment */
            (void)SomeIpTp_SendNextSegment(channelId);
        }
    }
}
...
Std_ReturnType SomeIpTp_BuildTpHeader(uint32 Offset, boolean MoreSegments, uint8* Buffer)
{
    ...
    if (Buffer != NULL_PTR)
    {
        SOMEIPTP_SET_TP_OFFSET(Buffer, Offset, MoreSegments);
        result = E_OK;
    }
...
Std_ReturnType SomeIpTp_ParseTpHeader(const uint8* Buffer, uint32* Offset, boolean* MoreSegments)
{
    ...
    if ((Buffer != NULL_PTR) && (Offset != NULL_PTR) && (MoreSegments != NULL_PTR))
    {
        SOMEIPTP_GET_TP_OFFSET(Buffer, *Offset, *MoreSegments);
        result = E_OK;
    }
...
Std_ReturnType SomeIpTp_GetRxBufferStatus(PduIdType RxPduId, PduLengthType* BufferSizePtr)
{
    Std_ReturnType result = E_NOT_OK;
    uint16 channelId;
    const SomeIpTp_RxBufferType* rxBufPtr;

#if (SOMEIPTP_DEV_ERROR_DETECT == STD_ON)
    if (BufferSizePtr == NULL_PTR)
    {
        SOMEIPTP_DET_REPORT_ERROR(SOMEIPTP_SID_MAINFUNCTION, SOMEIPTP_E_PARAM_POINTER);
        return E_NOT_OK;
    }
#endif
    ...
    if (SomeIpTp_FindChannelByRxPduId(RxPduId, &channelId) == E_OK)
    {
        rxBufPtr = &SomeIpTp_InternalState.Channels[channelId].RxBuffer;
        *BufferSizePtr = (PduLengthType)(rxBufPtr->MaxLength - rxBufPtr->Length);
        result = E_OK;
    }
