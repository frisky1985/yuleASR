                                      ...
                                      const StbM_UserDataType* userDataPtr)
{
    ...
    rxLocalTime = *virtualLocalTimePtr;

    /* Calculate rate deviation if we have previous sync point */
    if (tbPtr->timeValid)
    {
        sint64 localDiff = (sint64)(rxLocalTime - tbPtr->lastSyncLocalTime);
        sint64 globalDiff = ((sint64)(timeStampPtr->seconds - tbPtr->globalTime.seconds) * STBM_NS_PER_SECOND) +
                            (sint64)(timeStampPtr->nanoseconds - tbPtr->globalTime.nanoseconds);

        if (localDiff > 0)
        {
            tbPtr->rateDeviation = (StbM_RateDeviationType)(((globalDiff - localDiff) * 1000000LL) / localDiff);
        }
    }
    ...
    tbPtr->localTime = rxLocalTime;
    tbPtr->lastSyncLocalTime = rxLocalTime;
    tbPtr->timeValid = TRUE;
    tbPtr->syncStatus = STBM_SYNC_STATUS_SYNC;
    tbPtr->timeBaseStatus = STBM_TIMEBASE_STATUS_OK;
    tbPtr->timeoutCounter = 0U;
    tbPtr->updateCounter++;
                                       ...
                                       uint8* timeBaseStatusPtr)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASESTATUS, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASESTATUS, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }

    if ((syncStatusPtr == NULL_PTR) || (timeBaseStatusPtr == NULL_PTR))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASESTATUS, STBM_E_PARAM_POINTER);
        return E_NOT_OK;
    }
#endif
    ...
    *syncStatusPtr = tbPtr->syncStatus;
    *timeBaseStatusPtr = tbPtr->timeBaseStatus;
    result = E_OK;
                                     ...
                                     StbM_MasterConfigType* masterConfigPtr)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETMASTERCONFIG, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETMASTERCONFIG, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }

    if (masterConfigPtr == NULL_PTR)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETMASTERCONFIG, STBM_E_PARAM_POINTER);
        return E_NOT_OK;
    }
#endif
    ...
    if (StbM_FindTimeBaseConfig(timeBaseId, &configPtr) == E_OK)
    {
        *masterConfigPtr = configPtr->masterConfig;
        result = E_OK;
    }
                                       ...
                                       StbM_RateDeviationType rateDeviation)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_SETRATECORRECTION, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_SETRATECORRECTION, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }
#endif
    ...
    if (StbM_FindTimeBaseConfig(timeBaseId, &configPtr) == E_OK)
    {
        ...
        if ((rateDeviation >= -(sint32)configPtr->allowedRateDeviation) &&
            (rateDeviation <= (sint32)configPtr->allowedRateDeviation))
        {
            tbPtr = &StbM_InternalState.TimeBases[timeBaseId];
            tbPtr->rateDeviation = rateDeviation;
            result = E_OK;
        }
    }
                                              ...
                                              uint32* updateCounterPtr)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASEUPDATECOUNTER, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASEUPDATECOUNTER, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }

    if (updateCounterPtr == NULL_PTR)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETTIMEBASEUPDATECOUNTER, STBM_E_PARAM_POINTER);
        return E_NOT_OK;
    }
#endif

    tbPtr = &StbM_InternalState.TimeBases[timeBaseId];
    *updateCounterPtr = tbPtr->updateCounter;
    result = E_OK;
                                        ...
                                        StbM_TimeDiffType* timeDiffPtr)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETCURRENTTIMEDIFF, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_GETCURRENTTIMEDIFF, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }

    if (timeDiffPtr == NULL_PTR)
    {
    ...
    tbPtr = &StbM_InternalState.TimeBases[timeBaseId];
    currentTime = StbM_GetVirtualLocalTime(timeBaseId);

    if (currentTime >= tbPtr->localTime)
    {
        timeDiffPtr->diff = (sint32)(currentTime - tbPtr->localTime);
    }
    else
    {
        timeDiffPtr->diff = -(sint32)(tbPtr->localTime - currentTime);
    }
                                 ...
                                 const StbM_UserDataType* userDataPtr)
{
    ...
    if (StbM_InternalState.State != STBM_STATE_INIT)
    {
        STBM_DET_REPORT_ERROR(STBM_SID_SETUSERDATA, STBM_E_UNINIT);
        return E_NOT_OK;
    }

    if (!STBM_IS_VALID_TIMEBASE_ID(timeBaseId))
    {
        STBM_DET_REPORT_ERROR(STBM_SID_SETUSERDATA, STBM_E_INVALID_TIMEBASE_ID);
        return E_NOT_OK;
    }
...
void StbM_TimeStampChanged(uint8 timeBaseId, const Eth_TimeStampType* ethTimeStampPtr)
{
    ...
    /* Convert Eth timestamp to StbM timestamp */
    stbTime.seconds = ethTimeStampPtr->seconds;
    stbTime.nanoseconds = ethTimeStampPtr->nanoseconds;
    stbTime.secondsHi = 0U;

    /* Update global time */
    tbPtr->globalTime = stbTime;
    tbPtr->localTime = StbM_GetVirtualLocalTime(timeBaseId);
    tbPtr->timeValid = TRUE;
    tbPtr->syncStatus = STBM_SYNC_STATUS_SYNC;
    tbPtr->timeBaseStatus = STBM_TIMEBASE_STATUS_OK;
