 ...
 *                                    VERSION CHECK
 *==================================================================================================*/
#if (SECOC_AR_RELEASE_MAJOR_VERSION != 4u)
    #error "SecOC.c: AR major version mismatch"
#endif

#if (SECOC_AR_RELEASE_MINOR_VERSION != 7u)
...
Std_ReturnType SecOC_IfTransmit(PduIdType TxPduId, const PduInfoType* PduInfoPtr)
{
    ...
    /* Copy PDU data to buffer */
    SecOC_TxBuffers[idx].length = (PduInfoPtr->SduLength < SECOC_MAX_PDU_LENGTH) ? 
                                   PduInfoPtr->SduLength : SECOC_MAX_PDU_LENGTH;
    
    for (i = 0u; i < (uint16)SecOC_TxBuffers[idx].length; i++) {
        SecOC_TxBuffers[idx].data[i] = PduInfoPtr->SduDataPtr[i];
    }
    
    SecOC_TxBuffers[idx].inUse = TRUE;
    SecOC_TxPduState[idx].txInProgress = TRUE;
    
    SchM_Exit_SecOC_SECOC_EXCLUSIVE_AREA_0();
    
    /* Process and transmit immediately (or in MainFunctionTx) */
    return SecOC_ProcessTxPdu(TxPduId);
}
...
void SecOC_IfRxIndication(PduIdType RxPduId, const PduInfoType* PduInfoPtr)
{
    ...
    /* Copy PDU data to buffer */
    SecOC_RxBuffers[idx].length = (PduInfoPtr->SduLength < SECOC_MAX_PDU_LENGTH) ? 
                                   PduInfoPtr->SduLength : SECOC_MAX_PDU_LENGTH;
    
    for (i = 0u; i < (uint16)SecOC_RxBuffers[idx].length; i++) {
        SecOC_RxBuffers[idx].data[i] = PduInfoPtr->SduDataPtr[i];
    }
    
    SecOC_RxBuffers[idx].inUse = TRUE;
    SecOC_RxPduState[idx].timeoutCounter = 0u;
    
    SchM_Exit_SecOC_SECOC_EXCLUSIVE_AREA_0();
    
    /* Process received PDU */
    SecOC_ProcessRxPdu(RxPduId);
}
...
#if (SECOC_DEV_ERROR_DETECT == STD_ON)
    if (SecOC_Initialized_Local == FALSE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, SECOC_SID_VERIFYSTATUSOVERRIDE, 
                               SECOC_E_UNINIT);
        return E_NOT_OK;
    }
    
    if (status > SECOC_VERIFICATIONOVERRIDE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, SECOC_SID_VERIFYSTATUSOVERRIDE, 
                               SECOC_E_INVALID_PARAMETER);
        return E_NOT_OK;
    }
#endif

#if (SECOC_OVERRIDE_STATUS_ALLOWED == STD_OFF)
    (void)PduId;
    (void)status;
    return E_NOT_OK;
#else
...
SecOC_VerificationStatusType SecOC_GetVerificationStatus(PduIdType PduId)
{
...
#if (SECOC_DEV_ERROR_DETECT == STD_ON)
    if (SecOC_Initialized_Local == FALSE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, 0x86u, SECOC_E_UNINIT);
    ...
    SchM_Enter_SecOC_SECOC_EXCLUSIVE_AREA_0();
    status = SecOC_RxPduState[idx].status;
    SchM_Exit_SecOC_SECOC_EXCLUSIVE_AREA_0();
...
#if (SECOC_DEV_ERROR_DETECT == STD_ON)
    if (SecOC_Initialized_Local == FALSE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, SECOC_SID_TXCONFIRMATION, 
                               SECOC_E_UNINIT);
        return;
    }
#endif
...
void SecOC_MainFunctionRx(void)
{
...
#if (SECOC_DEV_ERROR_DETECT == STD_ON)
    if (SecOC_Initialized_Local == FALSE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, SECOC_SID_MAINFUNCTIONRX, 
                               SECOC_E_UNINIT);
        return;
    }
#endif
    ...
    /* Process pending RX PDUs */
    for (i = 0u; i < SECOC_NUM_RX_PDUS; i++) {
        if (SecOC_RxBuffers[i].inUse && !SecOC_RxPduState[i].authInProgress) {
            SecOC_RxPduState[i].timeoutCounter += SECOC_MAIN_FUNCTION_PERIOD_RX_MS;
            
            if (SecOC_RxPduState[i].timeoutCounter >= SECOC_VERIFICATION_TIMEOUT_MS) {
                /* Timeout - mark as failed */
                SecOC_RxPduState[i].status = SECOC_VERIFICATIONFAILURE_STATUS;
                SecOC_RxPduState[i].lastResult = SECOC_VERIFICATIONFAILURE;
                SecOC_RxBuffers[i].inUse = FALSE;
            } else {
                /* Retry verification */
                SecOC_ProcessRxPdu(i);
            }
        }
    }
...
void SecOC_MainFunctionTx(void)
{
...
#if (SECOC_DEV_ERROR_DETECT == STD_ON)
    if (SecOC_Initialized_Local == FALSE) {
        (void)Det_ReportError(SECOC_MODULE_ID, SECOC_INSTANCE_ID, SECOC_SID_MAINFUNCTIONTX, 
                               SECOC_E_UNINIT);
        return;
    }
#endif
    ...
    /* Process pending TX PDUs */
    for (i = 0u; i < SECOC_NUM_TX_PDUS; i++) {
        if (SecOC_TxBuffers[i].inUse && SecOC_TxPduState[i].txInProgress) {
            (void)SecOC_ProcessTxPdu(i);
        }
    }
...
/* Stub functions for TP PDUs - would be fully implemented for TP support */
BufReq_ReturnType SecOC_CopyTxData(PduIdType TxPduId, PduInfoType* PduInfoPtr,
...
BufReq_ReturnType SecOC_CopyRxData(PduIdType RxPduId, const PduInfoType* PduInfoPtr,
...
BufReq_ReturnType SecOC_StartOfReception(PduIdType RxPduId, PduLengthType TpSduLength,
                                          PduLengthType* RxBufferSizePtr)
{
    (void)RxPduId;
    (void)TpSduLength;
