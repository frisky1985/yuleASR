...
/**
 * @file LinMaster_Tp.c
 * @brief LIN Master TP (传输协议) 客户端实现 - ISO 17987
 * @version 1.0.0
 */

#include "LinMaster_Tp.h"
#include "LinSlave_Hal.h"  /* 重用HAL层 */
#include <string.h>
...
/* 流控状态 */
#define LINMASTER_TP_FC_CTS             0x00    /* Continue To Send */
#define LINMASTER_TP_FC_WT              0x01    /* Wait */
#define LINMASTER_TP_FC_OVFLW           0x02    /* Overflow */
...
static LinMaster_Tp_StatusType LinMaster_Tp_ProcessFlowControl(uint8* DataPtr, uint8 Length);
...
static LinMaster_Tp_StatusType LinMaster_Tp_ProcessFlowControl(uint8* DataPtr, uint8 Length)
{
    ...
    switch (FlowStatus) {
        case LINMASTER_TP_FC_CTS:
            ...
            /* 状态转换到发送连续帧 */
            if (TpChannel.State == LINMASTER_TP_STATE_WAIT_FC ||
                TpChannel.State == LINMASTER_TP_STATE_TX_FF) {
        ...
        case LINMASTER_TP_FC_WT:
            /* 等待 - 重置超时计时器 */
            TpChannel.LastFrameTime = LinSlave_Hal_GetTimestampMs();
            TpChannel.Timer = 0;
            break;
            
        case LINMASTER_TP_FC_OVFLW:
            /* 溢出错误 - 取消传输 */
            if (TpTxConfirm != NULL_PTR) {
...
void LinMaster_Tp_MainFunction(void)
{
    ...
    switch (TpChannel.State) {
        case LINMASTER_TP_STATE_IDLE:
            /* 空闲状态，无需处理 */
            break;
            
        case LINMASTER_TP_STATE_TX_SF:
            /* 单帧发送完成，检查超时 */
        ...
        case LINMASTER_TP_STATE_WAIT_FC:
            /* 等待流控帧超时检查 */
            if (ElapsedTime > LINMASTER_TP_N_BS_TIMEOUT) {
                if (TpTxConfirm != NULL_PTR) {
                    TpTxConfirm(TpChannel.ChannelId, LINMASTER_TP_E_TIMEOUT);
