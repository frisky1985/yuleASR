...
/*******************************************************************************
 * @file    E2E_P04.c
 * @brief   E2E Profile 4 Implementation - CRC32
 * @details Implements 32-bit CRC (Ethernet standard) for E2E protection
 * @author  AutoSAR Team
 * @version 1.0.0
 ******************************************************************************/
...
/*=============================================================================*
 * Local CRC32 Lookup Table (Ethernet polynomial 0x04C11DB7)
 *=============================================================================*/
static const uint32 E2E_P04_CRC32_Table[256] = {
 ...
 *=============================================================================*/
static uint32 E2E_P04_CalculateCRC(
    const uint8* Data,
    uint32 Length,
    uint32 DataID,
...
/**
 * @brief Calculate CRC32 for Profile 4
 */
static uint32 E2E_P04_CalculateCRC(
    const uint8* Data,
    uint32 Length,
...
)
{
    ...
    /* CRC over data (excluding CRC field) */
    for (i = 0U; i < Length; i++) {
        crc = E2E_P04_CRC32_Table[(crc >> 24) ^ Data[i]] ^ (crc << 8);
    }
    ...
    if (IncludeDataID) {
        ...
        crc = E2E_P04_CRC32_Table[(crc >> 24) ^ (uint8)((DataID >> 24) & 0xFFU)] ^ (crc << 8);
    }
...
)
{
    ...
    /* Write 16-bit counter (big-endian) */
    Data[Config->CounterOffset] = (uint8)((State->Counter >> 8) & 0xFFU);
    Data[Config->CounterOffset + 1U] = (uint8)(State->Counter & 0xFFU);
    
    /* Clear CRC field before calculation */
    for (i = 0U; i < 4U; i++) {
        Data[Config->CRCOffset + i] = 0U;
    }
    
    /* Calculate CRC32 over entire data */
    crc = E2E_P04_CalculateCRC(Data, Config->DataLength, Config->DataID, Config->IncludeDataID);
    
    /* Write CRC (big-endian) */
    Data[Config->CRCOffset] = (uint8)((crc >> 24) & 0xFFU);
    Data[Config->CRCOffset + 1U] = (uint8)((crc >> 16) & 0xFFU);
    Data[Config->CRCOffset + 2U] = (uint8)((crc >> 8) & 0xFFU);
    Data[Config->CRCOffset + 3U] = (uint8)(crc & 0xFFU);
...
)
{
    ...
    /* Read received CRC (big-endian) */
    receivedCrc = ((uint32)Data[Config->CRCOffset] << 24) |
                  ((uint32)Data[Config->CRCOffset + 1U] << 16) |
                  ((uint32)Data[Config->CRCOffset + 2U] << 8) |
                  (uint32)Data[Config->CRCOffset + 3U];
    ...
    tempData[Config->CRCOffset + 3U] = 0U;
    
    /* Calculate expected CRC */
    calculatedCrc = E2E_P04_CalculateCRC(tempData, Config->DataLength, Config->DataID, Config->IncludeDataID);
    ...
    /* Read counter (big-endian) */
    receivedCounter = ((uint16)Data[Config->CounterOffset] << 8) |
                      (uint16)Data[Config->CounterOffset + 1U];
    ...
    if (State->WaitForFirstData == 0U) {
        ...
        if (delta == 0) {
            ...
            State->LastValidCounter = receivedCounter;
        } else if ((uint32)delta <= (uint32)State->MaxDeltaCounterInit) {
