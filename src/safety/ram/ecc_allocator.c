...
static EccMemBlockType* EccAllocator_FindFreeBlock(uint32_t size, uint32_t alignment)
{
    ...
    while (current != NULL) {
        ...
        if (current->header.actual_size >= size) {
            /* Check alignment */
            uint32_t user_ptr = (uint32_t)(uintptr_t)EccAllocator_BlockToPtr(current);
...
static void EccAllocator_SetGuards(EccMemBlockType *block)
{
    ...
    user_data = (uint8_t*)EccAllocator_BlockToPtr(block) + block->header.front_offset;
    
    /* Front guard (before user data) */
    if (block->header.flags & ECC_ALLOC_FLAG_GUARD) {
        front_guard = (uint32_t*)(user_data - sizeof(uint32_t));
...
static void* EccAllocator_DoAlloc(uint32_t size, uint32_t alignment, uint32_t flags)
{
    ...
    /* Compute user data start: after header, plus front guard if enabled.
     * front_offset records (guard word + alignment padding) so that
     * PtrToBlock() can recover the block header from the user pointer. */
    user_ptr = EccAllocator_BlockToPtr(block);
    block->header.front_offset = 0U;
    if (flags & ECC_ALLOC_FLAG_GUARD) {
        user_ptr = (uint8_t*)user_ptr + sizeof(uint32_t);
        block->header.front_offset = (uint8_t)sizeof(uint32_t);
    }
    if (alignment > 1U) {
        uintptr_t user_addr = (uintptr_t)user_ptr;
        uintptr_t aligned_addr = (user_addr + (uintptr_t)(alignment - 1U)) & ~((uintptr_t)alignment - 1U);
        padding = (uint32_t)(aligned_addr - user_addr);
        if (padding > 0U) {
            user_ptr = (void*)aligned_addr;
            block->header.front_offset = (uint8_t)(block->header.front_offset + padding);
        }
    }
...
Std_ReturnType EccAllocator_Init(const EccHeapConfigType *config)
{
    ...
    /* Use default config if none provided or heap base not set */
    if (config != NULL) {
        g_heap_state.config = *config;
        /* Fall back to default heap if caller left base/size unset */
        if (g_heap_state.config.heap_base == NULL) {
            g_heap_state.config.heap_base = g_default_heap;
        }
        if (g_heap_state.config.heap_size == 0U) {
