...
typedef struct {
    ...
    uint8_t front_offset;       /* Bytes from header end to user data (guard+pad) */
 ...
 * Scans backward from the user pointer (up to 64 bytes) for the
 * block header magic.  Tolerates front-guard word and alignment
 * padding recorded in header.front_offset.
 */
static inline EccMemBlockType* EccAllocator_PtrToBlock(void *ptr)
{
    ...
    for (i = 0U; i <= 16U; i++) {
        EccMemBlockType *cand = (EccMemBlockType*)(p - sizeof(EccMemBlockHeaderType) - (i * 4U));
        if (cand->header.magic == ECC_ALLOC_BLOCK_MAGIC) {
            return cand;
        }
    }
    return NULL;
