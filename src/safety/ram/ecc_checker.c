...
static const uint8_t ecc7_syndrome_map[128] = {
    /* Syndrome 0-127 mapping to bit positions (regenerated to match matrix) */
    
};
...
static EccCheckerType g_checker_state;
...
{
    EccCheckerConfigType default_config;

    if (config == NULL) {
        /* Use defaults: auto-correct single-bit errors */
        memset(&default_config, 0, sizeof(default_config));
        default_config.auto_correct = TRUE;
...
/**
 * @brief Popcount helper for syndrome classification
 */
static uint8_t EccChecker_PopCount(uint8_t value)
{
    uint8_t count = 0U;
    while (value != 0U) {
                                             ...
                                             EccCheckResultType *result)
{
    ...
    /* Single-bit syndrome: ECC code itself corrupted - data intact */
    ...
    /* Unmapped syndrome: even popcount = double-bit, odd = multi-bit */
    if ((EccChecker_PopCount(syndrome) & 1U) == 0U) {
...
/**
 * @brief Check if an error type is uncorrectable
 */
boolean EccChecker_IsUncorrectable(EccErrorType error_type)
{
    return (error_type == ECC_ERROR_DOUBLE_BIT) ||
           (error_type == ECC_ERROR_MULTI_BIT) ||
           (error_type == ECC_ERROR_ECC_CORRUPT);
}

/**
 * @brief Check and correct 32-bit data (status-oriented API)
 */
Std_ReturnType EccChecker_Check32(uint32_t data, uint8_t ecc_code, EccCheckedType* result)
{
    EccCheckResultType detail;
    Std_ReturnType status;
    ...
    status = EccChecker_CheckAndCorrect32(data, ecc_code, &detail);

    result->data = detail.corrected_data;
    result->data64 = detail.corrected_data64;
    result->ecc_code = ecc_code;
    result->syndrome = detail.syndrome;
    result->error_position = detail.error_position;

    switch (detail.error_type) {
        case ECC_ERROR_NONE:
            result->result = ECC_CHECK_OK;
            break;
        case ECC_ERROR_SINGLE_BIT:
            result->result = ECC_CHECK_SINGLE_ERROR;
            break;
        case ECC_ERROR_DOUBLE_BIT:
...
/**
 * @brief Check and correct 64-bit data (status-oriented API)
 */
Std_ReturnType EccChecker_Check64(uint64_t data, uint8_t ecc_code, EccCheckedType* result)
{
    ...
    status = EccChecker_CheckAndCorrect64(data, ecc_code, &detail);

    result->data = (uint32_t)detail.corrected_data64;
    result->data64 = detail.corrected_data64;
    result->ecc_code = ecc_code;
    result->syndrome = detail.syndrome;
    result->error_position = detail.error_position;
