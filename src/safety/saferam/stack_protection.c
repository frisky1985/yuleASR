...
{
#if defined(__CORTEX_M) || defined(__ARM_ARCH_7M__) || defined(__ARM_ARCH_7EM__) || defined(__ARM_ARCH_6M__) || (defined(__arm__) && !defined(__arm64__))
    register uint32_t result;
    __asm volatile ("MRS %0, msp" : "=r" (result));
    return result;
#else
    /* Host build (non-Cortex-M): no MSP register available */
    return 0U;
#endif
}
...
uint32_t StackProtection_GetPSP(void)
{
    ...
    return result;
#else
    /* Host build (non-Cortex-M): no PSP register available */
    return 0U;
#endif
}
