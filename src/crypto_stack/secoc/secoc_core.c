...
static void left_shift_one(const uint8_t *in, uint8_t *out) {
    ...
    for (int i = AES_BLOCK_SIZE - 1U; i >= 0; i--) {
        out[i] = (uint8_t)((in[i] << 1U) | overflow);
...
static void sha256_final(uint32_t state[8], uint8_t *buffer, uint32_t *buf_len, uint64_t *bit_len, uint8_t hash[32]) {
    ...
    i++;
    if (i > 56U) {
        while (i < 64U) { buffer[i] = 0; i++; }
        sha256_transform(state, buffer);
        i = 0;
    }
    while (i < 56U) { buffer[i] = 0; i++; }
...
static void aes_cmac(const uint8_t *key, uint32_t key_len,
    ...
    uint32_t n = (msg_len + AES_BLOCK_SIZE - 1U) / AES_BLOCK_SIZE;
    ...
    if (!flag) {
        /* 完整块 */
        for (uint32_t j = 0U; j < AES_BLOCK_SIZE; j++) {
            m_last[j] = msg[((n - 1U) * AES_BLOCK_SIZE) + j] ^ k1[j];
        }
    } else {
        /* 不完整块 */
        for (uint32_t j = 0U; j < AES_BLOCK_SIZE; j++) {
            if (j < (uint32_t)(msg_len % AES_BLOCK_SIZE)) {
                m_last[j] = msg[((n - 1U) * AES_BLOCK_SIZE) + j];
            } else if (j == (uint32_t)(msg_len % AES_BLOCK_SIZE)) {
                m_last[j] = 0x80;
            } else {
                m_last[j] = 0;
            }
    ...
    for (uint32_t i = 0U; i < (n - 1U); i++) {
        for (uint32_t j = 0U; j < AES_BLOCK_SIZE; j++) {
            y[j] = x[j] ^ msg[((uint32_t)i * AES_BLOCK_SIZE) + j];
        }
...
secoc_context_t* secoc_init(dds_crypto_context_t *crypto_ctx) {
    ...
    /* 初始化密钥槽 */
    for (uint32_t i = 0U; i < SECOC_MAX_KEY_SLOTS; i++) {
...
secoc_status_t secoc_remove_pdu_config(secoc_context_t *ctx, uint32_t pdu_id) {
    ...
    for (uint32_t i = 0; i < ctx->num_pdu_configs; i++) {
        if (ctx->pdu_configs[i].pdu_id == pdu_id) {
            /* 移除配置 */
            memmove(&ctx->pdu_configs[i], &ctx->pdu_configs[i + 1U],
                    (ctx->num_pdu_configs - i - 1U) * sizeof(secoc_pdu_config_t));
            ctx->num_pdu_configs--;
            return SECOC_OK;
        }
    }
...
secoc_status_t secoc_compute_mac(secoc_context_t *ctx, const secoc_pdu_config_t *config,
    ...
    /* 添加数据 */
    if (auth_pdu->data && (auth_pdu->data_len > 0U)) {
        memcpy(auth_data, auth_pdu->data, auth_pdu->data_len);
        auth_len += auth_pdu->data_len;
    }
    
    /* 添加Freshness值 */
    uint8_t fv_len = config->freshness_value_len / 8U;
    if ((fv_len > 0U) && (fv_len <= SECOC_MAX_FRESHNESS_LENGTH)) {
...
secoc_status_t secoc_authenticate_tx_pdu(secoc_context_t *ctx, uint32_t pdu_id,
    ...
    tx_counter++;
    for (uint32_t i = 0U; (i < (uint32_t)auth_pdu->freshness_len) && (i < 8U); i++) {
        auth_pdu->freshness_value[i] = (uint8_t)((freshness >> (i * 8U)) & 0xFFU);
    }
