...
/* 由 dds_security_manager.c 提供 (避免重复实现时间源) */
...
dds_access_context_t* dds_access_init(const dds_security_config_t *config)
{
    ...
    if ((ctx->permissions_ca_cert_path[0]) != 0) {
                                                   ...
                                                   const dds_access_request_t *request)
{
    ...
    if ((request->asil_level < ctx->default_policy.rules ? ctx->default_policy.rules->min_asil_level : DDS_SECURITY_ASIL_QM) != 0) {
                                                      ...
                                                      dds_governance_config_t *governance)
{
    ...
    if ((ctx->governance_file_path[0]) != 0) {
