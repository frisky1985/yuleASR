/***********************************************************************************************************************
 * File:        sha3_256.c
 * Description: SHA-3-256 implementation (FIPS 202, 256-bit digest)
 *              Keccak-f[1600] with 512-bit capacity and 1088-bit rate
 **********************************************************************************************************************/
...
/* SHA3-256 parameters */
#define SHA3_256_BLOCK_SIZE         136U    /* 1088 bits = 17 * 64-bit lanes */
#define SHA3_256_DIGEST_SIZE        32U     /* 256 bits */
#define SHA3_256_CAPACITY           64U     /* 512 bits */

/* Keccak Round Constants (same for all SHA3 variants) */
...
void SHA3_256_Update(SHA3_256_Context* ctx, const uint8_t* data, uint32_t len)
{
    const uint8_t* d = data;
    uint32_t blocks;

    if (ctx->bufLen > 0) {
        uint32_t need = SHA3_256_BLOCK_SIZE - ctx->bufLen;
        if (len < need) {
        ...
        sha3_absorb(ctx->state, ctx->buffer, 1, SHA3_256_BLOCK_SIZE);
        d += need;
        len -= need;
        ctx->bufLen = 0;
    }

    blocks = len / SHA3_256_BLOCK_SIZE;
    if (blocks > 0) {
        sha3_absorb(ctx->state, d, blocks, SHA3_256_BLOCK_SIZE);
        d += blocks * SHA3_256_BLOCK_SIZE;
        len -= blocks * SHA3_256_BLOCK_SIZE;
    }
...
void SHA3_256_Final(SHA3_256_Context* ctx, uint8_t digest[32])
{
    uint32_t padLen = SHA3_256_BLOCK_SIZE - ctx->bufLen;

    ctx->buffer[ctx->bufLen] = 0x06;  /* SHA3 domain separator */
    memset(ctx->buffer + ctx->bufLen + 1, 0, padLen - 2);
    ctx->buffer[SHA3_256_BLOCK_SIZE - 1] = 0x80;

    sha3_absorb(ctx->state, ctx->buffer, 1, SHA3_256_BLOCK_SIZE);
    sha3_squeeze(ctx->state, digest, SHA3_256_DIGEST_SIZE, SHA3_256_BLOCK_SIZE);

    memset(ctx, 0, sizeof(*ctx));
}

void SHA3_256(const uint8_t* data, uint32_t len, uint8_t digest[32])
{
