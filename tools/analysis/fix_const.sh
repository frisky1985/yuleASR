...
sed -i '' 's/Rte_PortStateType\* portState = \&Rte_ComponentStates\[componentId\]\.Ports\[portId\];/const Rte_PortStateType* portState = \&Rte_ComponentStates[componentId].Ports[portId];/' "src/middleware/rte/src/Rte.c" && echo "Fixed Rte.c:447"
...
sed -i '' 's/Mqtt_DdsDomainStateType\* state = &g_domain->state;/const Mqtt_DdsDomainStateType* state = \&g_domain->state;/' "src/middleware/microdds/src/core/domain.c" && echo "Fixed domain.c:148"

# 35-36. micro-dds/src/core/topic.c:135,149  
sed -i '' 's/Mqtt_DdsTopicStateType\* state = &g_topicManager->state;/const Mqtt_DdsTopicStateType* state = \&g_topicManager->state;/g' "src/middleware/microdds/src/core/topic.c" && echo "Fixed topic.c:135,149"

# 37. micro-dds/src/core/reader.c:133
sed -i '' 's/Mqtt_DdsReaderStateType\* state = &g_readerManager->state;/const Mqtt_DdsReaderStateType* state = \&g_readerManager->state;/' "src/middleware/microdds/src/core/reader.c" && echo "Fixed reader.c:133"

# 38. src/middleware/microdds/tests/unity/test_buffer_pool.c:56
sed -i '' 's/uint8_t\* extra;/const uint8_t* extra;/' "src/middleware/microdds/tests/unity/test_buffer_pool.c" && echo "Fixed test_buffer_pool.c:56"

# 39. src/transport/udp.c:1229
sed -i '' 's/uint8_t\* p;/const uint8_t* p;/' "src/middleware/microdds/src/transport/udp.c" && echo "Fixed udp.c:1229"
...
# 44. reader.c:126
sed -i '' 's/Dds_SampleInfoType\*\* sample_infos/const Dds_SampleInfoType** sample_infos/' "src/middleware/microdds/src/core/reader.c" && echo "Fixed reader.c:126"

# 45. buffer_pool.c:109
sed -i '' 's/uint8_t\* buffer/const uint8_t* buffer/' "src/middleware/microdds/src/utils/buffer_pool.c" && echo "Fixed buffer_pool.c:109"
